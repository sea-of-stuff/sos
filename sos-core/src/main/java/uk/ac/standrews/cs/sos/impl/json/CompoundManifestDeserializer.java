...
public class CompoundManifestDeserializer extends JsonDeserializer<Compound> {
    ...
    public Compound deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException {
        ...
        try {
            ...
            return new CompoundManifest(compoundType, contentGUID, contents, signatureRole.Y(), signatureRole.X());

        } catch (GUIDGenerationException e) {
            throw new IOException("Unable to recreate GUID");
        } catch (RoleNotFoundException e) {
            throw new IOException(("Unable to get signer for compound manifest"));
        }
